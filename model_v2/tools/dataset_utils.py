...
class BaseDataSet(torch.utils.data.Dataset):
    def __init__(self, city_base_info, size, crime_dataset_all, need_shuffle=False, self_adjustment=False, p_ratio=0.5):
        super().__init__()
        # self.grid_time_dim = grid_time_dim  # 限定grid_time的时间维长度
    ...
    def _generate_data(self):
        ...
        # time_type_for_query_mask = np.zeros((len(query_time), time_type_mask.shape[0], time_type_mask.shape[1]))
        ...
        crime_one_hot = np.zeros(self.n_crime_type)  # 暂时没用
