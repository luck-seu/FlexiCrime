...
class BaseDataSet(torch.utils.data.Dataset):
    def __init__(self, city_base_info, size, crime_dataset_all, predict_crime_id=-1, need_shuffle=False,
                 self_adjustment=False, p_ratio=0.5, pp_data=-1, case=False):  # pp_data为点过程数据, case为构建输出事件特征的数据集
        super().__init__()
        ...
        # 时间为x，类型为y，值为crime one-hot 和 grid feature 拼接 -> 多类别已启用
        ...
        self.has_pp = False
        if pp_data != -1:
            self._update_crime_dataset(pp_data)
            self.has_pp = True
    ...
    def _update_crime_dataset(self, pp_data):
        # pp_data 放缩到0-1上
        min_max_scaler = MinMaxScaler()
        pp_len = len(pp_data)
        pp_data = np.array(pp_data)
        all_pp = pp_data.reshape(pp_len, -1)
        min_max_scaler = min_max_scaler.fit(all_pp)
        pp_data = min_max_scaler.transform(pp_data.reshape(pp_len, -1)).reshape(pp_len).tolist()

        for i in range(len(self.crime_dataset)):
            n_pp = pp_data[i * 4: i * 4 + 4]
            self.crime_dataset[i].append(n_pp)
    ...
    def _normalization_time(self):
        ...
        for i in range(len(self.prediction_query_time_list)):
            ...
            self.prediction_query_time_list[i] = min_max_scaler.transform(
                self.prediction_query_time_list[i].reshape(now_len, -1)). \
                reshape(now_len)
    ...
    def _generate_data(self):
        ...
        if self.has_pp:
            in_t_s_id, in_t_e_id, q_t_id, query_grid, label, pp_labels = cd[0], cd[1], cd[2], cd[3], cd[4], cd[5]
        elif self.case:
            in_t_s_id, in_t_e_id, now_query_time, query_grid, label = cd[0], cd[1], cd[2], cd[3], cd[4]
        else:
            in_t_s_id, in_t_e_id, q_t_id, query_grid, label = cd[0], cd[1], cd[2], cd[3], cd[4]

        if self.case:
            query_time = now_query_time
            prediction_query_time = np.array([])  # 占位
        else:
        ...
        all_time_type_for_query_mask = np.zeros(
            (len(query_time), len(query_time[0]), time_type_mask.shape[0], time_type_mask.shape[1]))
        for q_i in range(len(query_time)):
            query_reference_time = query_time[q_i]
            time_type_for_query_mask = np.zeros(
                (len(query_reference_time), time_type_mask.shape[0], time_type_mask.shape[1]))
        ...
        if self.has_pp:
            return reference_time_type, all_time_type_for_query_mask, key_time, key_grid, query_time, np.array(
                query_grid), \
                query_grid_feature, prediction_query_time, np.array(label), np.array(pp_labels)
        else:
            return reference_time_type, all_time_type_for_query_mask, key_time, key_grid, query_time, np.array(
                query_grid), \
...
class FineTuningBaseDataSet(torch.utils.data.Dataset):
    def __init__(self, city_base_info, size, crime_dataset_all, pp_feature, crime_dataset_all_count=None,
                 predict_crime_id=-1, need_shuffle=False,
                 self_adjustment=False, p_ratio=0.5):  # 微调版，数据集中携带id，从pp_feature中去取来进行构建数据集
        super().__init__()
        self.predict_crime_id = predict_crime_id
        self.n_lon_len, self.n_lat_len = city_base_info['n_lon_len'], city_base_info['n_lat_len']
        self.n_POI_cate = city_base_info['n_POI_cate']
        ...
        self.crime_dataset = crime_dataset_all[0]
        if crime_dataset_all_count is not None:
            self.crime_dataset_count = crime_dataset_all_count[0]
        self.query_time_list = crime_dataset_all[1]
        self.prediction_query_time_list = crime_dataset_all[2]
        self.pp_feature = pp_feature
        self._normalization_pp_feature()  # 归一化pp_feature
    ...
    def _normalization_pp_feature(self):
        min_max_scaler = MinMaxScaler()
        original_dict = self.pp_feature
        # 对字典中的值进行归一化

        # 对于每一个list进行max_min归一化
        normalized_dict = {
            key: [min_max_scaler.fit_transform(np.array(sublist).reshape(-1, 1)).reshape(-1).tolist() for sublist in
                  value]
            for key, value in original_dict.items()}

        self.pp_feature = normalized_dict
        pass
    ...
    def _generate_data(self):
        id = self._get_generate_id()
        cd = self.crime_dataset[id]
        in_t_s_id, in_t_e_id, q_t_id, query_grid, label, pp_id = cd[0], cd[1], cd[2], cd[3], cd[4], cd[5]
        label_count = []
        if self.crime_dataset_count is not None:
            label_count = self.crime_dataset_count[id][4]
        ...
        grid_feature = self.POI_feature_cate[self._get_grid_id(query_grid)]
        query_grid_feature = np.concatenate((crime_one_hot, grid_feature))
        grid_pp_feature = self.pp_feature[(query_grid[0], query_grid[1])][pp_id]

        return reference_time_type, all_time_type_for_query_mask, key_time, key_grid, query_time, np.array(
            query_grid), \
            query_grid_feature, prediction_query_time, np.array(label), np.array(grid_pp_feature), np.array(label_count)
