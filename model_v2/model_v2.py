...
class multi_attention_weight_decay(nn.Module):
    ...
    def forward(self, query_t, key_t, query_p, key_p, value, time_weight_decay, location_weight_decay,
        ...
        # 分别提取时空特征
        time_value = torch.sum(att_t_scores * value, -3).transpose(1, 2).contiguous().view(batch, query_time_len, f_dim, type_dim)
        time_value = self.new_transfer(self.merge(time_value).squeeze(-1))
        pos_value = torch.sum(att_p_scores * value, -3).transpose(1, 2).contiguous().view(batch, query_time_len, f_dim, type_dim)
        pos_value = self.new_transfer(self.merge(pos_value).squeeze(-1))

        return x, time_value, pos_value  # TODO:可以返回 time_value, pos_value 给点过程建模使用
...
class crime_prediction_weight_decay(nn.Module):
    def __init__(self, exp_args, device='cuda'):
        ...
        self.device = device
        self.nhidden = nhidden
        self.feature_hidden = exp_args.grid_feature_hidden + exp_args.crime_one_hot_hidden  # 只用grid 和 one-hot都用
        ...
        self.classifier_concatenate = nn.Sequential(
            nn.Linear(exp_args.query_prediction_time_len * nhidden + nhidden, nhidden), # 增加一个short-term的维度 or 考虑一下short-term的需不需要加长
            nn.LeakyReLU(),
            nn.Linear(nhidden, 32),
            nn.LeakyReLU(),
            nn.Linear(32, 2),
        )
        self.short_term_linear = nn.Sequential(
            nn.Linear(exp_args.query_prediction_time_len, nhidden),
            nn.LeakyReLU(),
    ...
    def forward(self, time_type_data, time_type_mask, time_steps, reference_time, time_type_grid, query_grid,
                query_grid_feature, prediction_out_concatenate=False, short_term_feature=False, get_crime_context=False):
        ...
        #  取时空context可以重新加个分支
        if get_crime_context:  # 取出对应事件的时空crime context
            n_reference_time = reference_time[:, 0, :]  # 就事件本身一个时间点
            n_time_type_mask = time_type_mask[:, 0, :, :, :]
            ...
            time_weight_decay = time_weight_decay.unsqueeze(-1).repeat(1, 1, 1,
                                                                       type_num)  # batch * reference_len * seq_len * type_num
            ...
            att_out, time_value, pos_value = self.att(query_time, key_time, query_pos, key_pos, time_type_val,
                                                      time_weight_decay,
                                                      location_weight_decay, n_time_type_mask)

            return time_value, pos_value
        else:  # 反之进行训练预测
            for i in range(sample_len):
                ...
                att_out, time_value, pos_value = self.att(query_time, key_time, query_pos, key_pos, time_type_val, time_weight_decay,
                                                      location_weight_decay, n_time_type_mask)
            ...
            if prediction_out_concatenate:  # 使用MA模型的特征和点过程补充的特征做预测
                final_out = all_out[0]
                for i in range(1, sample_len):
                    final_out = torch.cat([final_out, all_out[i]], -1)
                short_term_out = self.short_term_linear(short_term_feature)
                final_out = final_out.squeeze(0)
                final_out = torch.cat([final_out, short_term_out], -1) # 拼接短期特征
                return self.classifier_concatenate(final_out)
                ...
                return self.classifier(final_out.squeeze(0))
