...
import datasets_common as datasets
...
model_path = "/home/tangjun/CrimePrediction_LAB_PC/neural_stpp/trained_model/NYC/model0-1.pth"
crime_base_dataset = np.load('/home/tangjun/CrimePrediction_LAB_PC/neural_stpp/data/crimeNYC0/crime_nyc_theft_day3-1.npz')
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            for i in range(N):
                ...
                new_loc = np.array([datatest[i][1] - 3, datatest[i][2] - 2.5])
...
def task(tasklist, date, task_id, crime_id):
    ...
    with open(os.path.join('train_data/NYC/{}'.format(crime_id), date, write_name), 'wb') as f:
...
def get_dataset(crime_id, processes_num=20):
    ...
    write_dir = os.path.join('train_data/NYC/{}'.format(crime_id), dataset_date)
    os.makedirs(write_dir)

    with open('/home/tangjun/CrimePrediction/data/NYC/pp_data/point-process-query-{}-2016-2018.pkl'.format(crime_id), 'rb') as f:
...
def dataset_collection(crime_id, date, processes_num=20, to_file='', order=''):
    ...
    for i in range(processes_num):
        data_name = '/home/tangjun/CrimePrediction_LAB_PC/neural_stpp/train_data/NYC/{}/{}/crime{}-query-taskid-{}-{}.pkl'.format(crime_id, date+order, crime_id, i, date)
    ...
    if to_file == '':
        write_name = '/home/tangjun/CrimePrediction_LAB_PC/neural_stpp/train_data/NYC/{}/{}/all-crime{}-query-{}.pkl'.format(crime_id, date, crime_id, date)
...
if __name__ == '__main__':
    ...
    # data_name = '/home/tangjun/CrimePrediction_LAB_PC/data/NYC/pp_data/2023-4-26_15_30/pp-labels-10-2016-2018'
    # with open(data_name, 'rb') as f:
    #     pp_data = pickle.load(f)
    # f.close()
    # mi = torch.tensor(10000000)
    # mx = torch.tensor(-10000000)
    # sum = sum(pp_data)
    # for it in pp_data:
    #     mi = min(mi, it)
    #     mx = max(mx, it)
    # pp_data = sorted(pp_data)
    # print(mi, mx)
    # print(sum / len(pp_data), pp_data[len(pp_data) // 4], pp_data[len(pp_data) // 3], pp_data[len(pp_data) // 2],
    #       pp_data[len(pp_data) - (len(pp_data) // 3)], pp_data[len(pp_data) - (len(pp_data) // 5)])