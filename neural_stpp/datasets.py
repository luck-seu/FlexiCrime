...
class CrimeNYC(SpatioTemporalDataset):
    ...
    def __init__(self, datatest, split="train"):
        num = 4
        assert split in ["train", "val", "test"]
        self.split = split
        dataset = np.load("/home/tangjun/CrimePrediction/neural_stpp/data/crimeNYC01/crime_nyc_theft_hour.npz")
        data2test = datatest
        #data2test = [[pd.Timestamp("2016-01-02T00:00:00"), 1, 2], [pd.Timestamp("2016-01-08T00:00:00"), 3, 2], 
        ...
        #                       + dataset.files[4::30] + dataset.files[5::30] + dataset.files[6::30] + dataset.files[7::30]
        #                       + dataset.files[8::30] + dataset.files[9::30] + dataset.files[10::30])
        exclude_from_train = (dataset.files[::24] + dataset.files[1::24] + dataset.files[2::24] + dataset.files[3::24]
                             + dataset.files[4::24] + dataset.files[5::24] + dataset.files[6::24] + dataset.files[7::24])
        val_files = dataset.files[1::24]

        baseDate = pd.Timestamp("2016-01-01T00:00:00")
        for i in range(len(data2test)):
            t = (data2test[i][0] - baseDate).days * 4
        ...
        split_set = [dataset[f] for f in file_splits[split]]
        # if split == "test":
        #     split_set = [dataval[f] for f in file_splits[split]]
        super().__init__(train_set, split_set, split == "train")
...
# def spatiotemporal_events_collate_fn(data):
    ...
    # ])
    #
    # with open('./data/case-feature-crime-0-2016-2018.pkl', 'rb') as file:  # TODO:我这边好像需要对应修改，这样写的话
    ...
    # # time_context = torch.zeros(N, T, 64)
    # # space_context = torch.zeros(N, T, 64)
    #
    # # N1 = len(mask)
    # # T1 = len(mask[0])
    # # N2 = len(time_context)
    # # T2 = len(time_context[0])
    # # if N1!=N2 or T1!=T2:
    # #     print("???")
    #
