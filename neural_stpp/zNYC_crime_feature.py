...
# TODO: 需要对应修改
# model_path = "/home/hongyi/neural_stpp-theft/crime_model/model_10.pth"
# crime_base_dataset = np.load(
#     '/home/hongyi/neural_stpp-theft/data/pp_data/crime_nyc_10_24hour_15min_20160301-20180101.npz'
# )
...
def load_data(data, datatest, split="train"):
    if data == "citibike":
        ...
        # 这个可能要进行替换成现在的，而且我现在测试没有考虑使用。需要修改
        return datasets.CrimeNYC(crime_base_dataset, datatest, split=split)
    else:
...
def validate(model, test_loader, datatest, t0, t1, device, task_id=-1):
    ...
    #     time = math.floor(
    #         (datatest[i][0] - baseDate).total_seconds() % 86400 / 60 / 15)
    #     # time = (datatest[i][0] - baseDate).total_seconds() / 3600 / 24
    #     datatest[i][0] = (time - find_max(time, 15)) / 15 * 3  # 这里还需要修改吗
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            # # (yihong):构造两个数组放新的context
            # newtime_context = torch.zeros((N, T + 1, 64)).to(device)
            # newspace_context = torch.zeros((N, T + 1, 64)).to(device)
            # time_context = time_context.cpu()
            # space_context = space_context.cpu()

            # indexs = []

            # for i in range(N):
            #     index = get_index_of_last_element_smaller_than_x(
            #         event_times[i].reshape(-1),
            #         datatest[current_datatest_index][0])
            #     indexs.append(index)

            #     new_time = np.array(datatest[current_datatest_index][0])
            #     new_loc = np.array([
            #         datatest[current_datatest_index][1] - 3,
            #         datatest[current_datatest_index][2] - 2.5
            #     ])
            ...
            #                                  new_loc)
            #     current_datatest_index += 1

            #     # (yihong):把要插入的context找出来
            #     tc = np.array(time_context[i])
            #     tc = np.insert(tc,
            #                    index,
            #                    time_context[i][index - 1 if index -
            #                                    1 > 0 else index + 1],
            #                    axis=0)
            #     sc = np.array(space_context[i])
            #     sc = np.insert(sc,
            #                    index,
            #                    space_context[i][index - 1 if index -
            #                                     1 > 0 else index + 1],
            #                    axis=0)
            ...
            #     newevent_times, newspatial_locations, input_mask,
            #     newtime_context, newspace_context, t0, t1)
            intensitytime, space_loglik, time_loglik = model(
...
def get_different_1day():
    ...
    for id in range(len(all_query_points)):
        ...
        with open(os.path.join('./neural_stpp/data/NYC', write_name), 'wb') as f:
            pickle.dump(pp_q, f)
        f.close()
    pass
...
def get_1day_dataset(crime_id = -1):
    # crime_dict = {'THEFT': 0, 'ASSAULT 3 & RELATED OFFENSES': 1, 'HARRASSMENT 2': 2,
    #               'CRIMINAL MISCHIEF & RELATED OF': 3,
    #               'DANGEROUS DRUGS': 4, 'OFF. AGNST PUB ORD SENSBLTY &': 5, 'ROBBERY': 6, 'BURGLARY': 7,
    #               'VEHICLE AND TRAFFIC LAWS': 8, 'MISCELLANEOUS PENAL LAW': 9, 'DANGEROUS WEAPONS': 10,
    #               'OFFENSES AGAINST PUBLIC ADMINI': 11, 'SEX CRIMES': 12, 'FORGERY': 13, 'FRAUDS': 14,
    #               'CRIMINAL TRESPASS': 15,
    #               'POSSESSION OF STOLEN PROPERTY': 16, 'OFFENSES AGAINST THE PERSON': 17,
    #               'ADMINISTRATIVE CODE': 18, 'ARSON': 19,
    #               'NYS LAWS-UNCLASSIFIED VIOLATION': 20, 'OTHER STATE LAWS (NON PENAL LAW)': 21,
    #               'MURDER & NON-NEGL. MANSLAUGHTER': 22, 'GAMBLING': 23}
    n_lat_len, n_lon_len = 6, 7
    # hours_list = [4, 6, 8, 12, 24]
    ...
    for k in range(len(hours_list)):
        ...
        with open(os.path.join('./neural_stpp/data/NYC', data_name),
                  'rb') as f:
            pp_query = pickle.load(f)
        f.close()
        ...
        with open(
                './neural_stpp/train_data/NYC/{}/crime-pp-feature-{}hours.pkl'.
...
if __name__ == '__main__':
    ...
    crime_id = 0
    model_path = "/home/hongyi/neural_stpp-theft/crime_model/NYC/model_{}.pth".format(crime_id)
    crime_base_dataset = np.load(
        '/home/hongyi/neural_stpp-theft/data/NYC/pp_data/crime_nyc_{}_24hour_15min_20160301-20180101.npz'.format(crime_id)
    )
    model = torch.load(model_path)
    device = torch.device(f'cuda:0' if torch.cuda.is_available() else 'cpu')
