...
class CrimeNYC(SpatioTemporalDataset):
    ...
    def __init__(self, split="train"):
        assert split in ["train", "val", "test"]
        self.split = split
        dataset = np.load("data/crimeNYC0/crime_nyc_danger_day4.npz")
        data2test = np.load("data/crimeNYC0/crime_nyc_test_danger.npy", allow_pickle = True)
        #data2test = [["2016-01-02T00:00:00", 1, 2], ["2016-01-08T00:00:00", 3, 2], ["2016-02-24T00:00:00", 4, 1], ["2016-03-09T00:00:00", 2, 4]]
        testCollection = []

        exclude_from_train = (dataset.files[::30] + dataset.files[1::30] + dataset.files[2::30] + dataset.files[3::30]
                             + dataset.files[4::30] + dataset.files[5::30] + dataset.files[6::30] + dataset.files[7::30]
                             + dataset.files[8::30] + dataset.files[9::30] + dataset.files[10::30])
        val_files = dataset.files[1::30]

        baseDate = pd.Timestamp("2016-01-01T00:00:00")
        for i in range(len(data2test)):
            t = math.floor((data2test[i][0] - baseDate).days / 4)
...
class CrimeNYCTest(SpatioTemporalDataset):

    def __init__(self, datatest, split="train"):
        assert split in ["train", "val", "test"]
        self.split = split
        dataset = np.load("/home/tangjun/CrimePrediction/neural_stpp/data/crimeNYC10/crime_nyc_danger_day4.npz")
        data2test = datatest
        #data2test = [["2016-01-02T00:00:00", 1, 2], ["2016-01-08T00:00:00", 3, 2], ["2016-02-24T00:00:00", 4, 1], ["2016-03-09T00:00:00", 2, 4]]
        testCollection = []
